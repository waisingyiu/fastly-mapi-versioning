    ...
    for (let i = 0; i < this._str.length; i++) {
      ...
      if (
        ...
        if (charCode == "\/".charCodeAt(0)) {
          escaped.push(0x5c);
          escaped.push("/".charCodeAt(0));
        }
        if (charCode == "\b".charCodeAt(0)) {
          escaped.push(0x5c);
          escaped.push("b".charCodeAt(0));
        }
        if (charCode == "\n".charCodeAt(0)) {
          escaped.push(0x5c);
          escaped.push("n".charCodeAt(0));
        }
        if (charCode == "\r".charCodeAt(0)) {
          escaped.push(0x5c);
          escaped.push("r".charCodeAt(0));
        }
        if (charCode == "\t".charCodeAt(0)) {
          escaped.push(0x5c);
          escaped.push("t".charCodeAt(0));
        }
      } else {
        escaped.push(charCode);
      }
